...
export default function RoutingToolbar({ 
  ...
  return (
    <div className="relative flex flex-col items-center mt-2 animate-slide-up">
      <div className="flex items-center gap-1 bg-neutral-900 border border-white/10 rounded-full px-2 py-1.5 shadow-lg shadow-black/80">
        ...
        <div className="relative">
          <button 
            ...
            className={`p-2 rounded-full transition-all hover:bg-neutral-800 text-neutral-400 hover:text-brand-amber ${showFolderDropdown ? 'text-brand-amber bg-neutral-800' : ''}`}
          >
          ...
          {showFolderDropdown && (
            <div className="absolute bottom-12 left-0 z-50 w-56 bg-neutral-950 border border-white/10 rounded-md shadow-2xl py-1 animate-fade-in max-h-48 overflow-y-auto">
              <div className="px-3 py-1 text-xs text-neutral-500 font-semibold border-b border-white/5">Notes Folders</div>
              {userFolders.length === 0 ? (
                <div className="px-3 py-2 text-xs text-neutral-400 italic">No custom folders. Create one in Notes view.</div>
              ) : (
                userFolders.map(folder => (
                  <button
                    key={folder}
                    onClick={() => handleMoveToFolder(folder)}
                    className="w-full text-left px-3 py-1.5 text-xs text-neutral-300 hover:bg-brand-amber hover:text-black transition-colors truncate"
                  >
                    {folder}
                  </button>
                ))
        ...
        <button 
          ...
          className="p-2 rounded-full transition-all hover:bg-neutral-800 text-neutral-400 hover:text-brand-amber"
        >
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
